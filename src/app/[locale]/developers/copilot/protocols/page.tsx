    ...
    <div className="flex min-w-0 flex-col gap-8 xl:flex-row xl:items-start xl:gap-12">
      <div data-developer-doc-export-root className="min-w-0 w-full max-w-3xl flex-1">
        ...
        <section id="overview" className="mb-12">
          ...
          <p className="mb-4">
            The router&apos;s reach is built on <code className="rounded bg-gray-100 px-1.5 py-0.5 font-mono text-sm text-foreground">ILiquidityAdapter</code>{" "}
        ...
        <section id="responsibilities" className="mb-12">
          <h2 className="mb-4">Adapter Responsibilities</h2>
        ...
        <section id="adapters" className="mb-12">
          <h2 className="mb-4">Supported Adapters</h2>
          <p className="mb-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {protocols.map((protocol) => (
              <div
                key={protocol.name}
                className="flex flex-col rounded-lg border border-border bg-white p-4 transition-colors hover:border-cyan-200 hover:bg-cyan-50/40"
              >
                <div className="flex items-start gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-cyan-50 text-xs font-semibold text-type-accent">
                    {protocol.shortName}
                  </div>
                  <div className="min-w-0">
                    <h3 className="font-semibold leading-tight text-foreground">{protocol.name}</h3>
                    <span className="mt-0.5 inline-block text-xs font-medium uppercase tracking-wide text-type-accent">
                      {protocol.category}
                    </span>
                  </div>
                </div>
                <p className="mt-3">{protocol.purpose}</p>
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {protocol.features.map((feature) => (
                    <span
                      key={feature}
                      className="rounded-full border border-border bg-muted/40 px-2 py-0.5 text-xs text-type-secondary"
                    >
                      {feature}
                    </span>
                  ))}
