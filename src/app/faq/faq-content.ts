...
export const faqCategories: FaqCategory[] = [
    ...
    questions: [
      {
        id: "what-is-avana",
        q: "What is Avana?",
        a: "Avana is a lending protocol built for LP collateral. It lets users deposit supported AMM positions, keep those positions active in the underlying pool, and borrow against them through Aave v4 infrastructure.\n\nThe core idea is that LP capital does not have to become unusable just because it is inside a pool. Avana turns supported LP positions into collateral that can back credit while the original position continues to support swaps, earn fees, and remain tied to its AMM market."
      },
      {
        id: "what-problem-does-avana-solve",
        q: "What problem does Avana solve?",
        a: "Liquidity providers often have to remove liquidity before they can borrow against their capital. That means exiting the pool, giving up fee exposure, and interrupting the market position they already built.\n\nAvana solves this by making supported LP positions usable as collateral. The LP stays live, Avana tracks and values the position, and Aave v4 handles the borrow-side accounting through an internal vault collateral token."
      },
      {
        id: "how-is-avana-different-from-standard-lending",
        q: "How is Avana different from a standard lending market?",
  ...
  },
  {
    ...
    questions: [
      {
        id: "what-happens-when-i-deposit-lp-collateral",
        q: "What happens when I deposit LP collateral?",
        a: "When you deposit a supported LP position, Avana records the position and routes it into the relevant Borrow Spoke. The protocol verifies that the pool is approved, reconstructs the position, checks its risk settings, and calculates the collateral value attached to that market.\n\nOnce the position clears those checks, Avana mints the matching internal vault token for borrow-side accounting. Depositing does not automatically borrow assets; it creates borrowing capacity that the account can use while it remains healthy and the market has available liquidity."
      },
      {
        id: "does-my-lp-stay-active",
        q: "Does my LP position stay active after deposit?",
        a: "Yes. Supported LP positions stay active while they serve as collateral. The liquidity remains tied to the underlying AMM position, so the pool continues processing swaps and the position continues accruing fees and incentives according to the AMM rules.\n\nActions that change collateral value are checked by Avana while debt is open. Withdrawing liquidity, claiming fees, changing a range, or modifying the position runs through the Borrow Spoke because those actions affect the value backing the loan."
      },
      {
        id: "can-i-claim-lp-fees",
        q: "Can I claim LP fees while borrowing?",
        a: "Avana separates principal liquidity from accrued fees in its valuation model. That lets the protocol recognize fee income and release claimable fees while keeping LP principal active as collateral.\n\nFee claims pass through before-and-after health checks. If removing the fee value leaves the account under its required collateral boundary, Avana blocks the claim until the user repays debt or adds more collateral."
      },
      {
        id: "are-lp-fees-part-of-collateral-value",
        q: "Are LP fees included in collateral value?",
        a: "Accrued fees are tracked separately from principal liquidity. Avana’s oracle model distinguishes principal value from fee value, so the protocol understands how much of the position is core liquidity and how much is claimable fee income.\n\nThat distinction matters during liquidation. Avana applies accrued fees before principal liquidity when the market route supports it, reducing how much of the core LP position has to be unwound to cover debt."
      },
      {
        id: "when-can-i-withdraw-collateral",
        q: "When can I withdraw LP collateral?",
        a: "You can withdraw collateral after debt is fully repaid, or after Avana confirms that the remaining account still passes health checks after the withdrawal. Full repayment is the cleanest withdrawal path because the LP position no longer secures open debt.\n\nPartial collateral changes run through the Borrow Spoke. Avana recalculates the account after removing or modifying the requested LP position, then releases collateral only when the remaining approved collateral still supports the outstanding debt."
      }
    ]
  },
  {
    id: "valuation-oracles",
    name: "Valuation & Oracles",
    summary: "",
    questions: [
      {
        id: "how-does-avana-price-lp-collateral",
        q: "How does Avana price LP collateral?",
        a: "Avana prices LP collateral by reconstructing the position and valuing the assets inside it. For fungible LPs, the protocol derives value from external asset prices and pool balance reconstruction. For concentrated liquidity, it decomposes the position by liquidity, range, current tick, token exposure, and accrued fees.\n\nThe result is then discounted into recoverable collateral value. Borrow power is based on what the position can realistically support under the market’s risk assumptions, not on an optimistic net asset value."
      },
      {
        id: "how-does-avana-reduce-oracle-risk",
        q: "How does Avana reduce oracle and pricing risk?",
        a: "Avana uses a dual-oracle pricing framework for LP collateral. Chainlink price feeds provide the primary reference for the underlying assets, while AMM-derived TWAPs act as an independent verification layer sourced from onchain liquidity.\n\nBorrowing power is granted only when the pricing sources stay within the market’s tolerance band. This reduces exposure to flash-loan manipulation, transient AMM price distortions, stale updates, and bad collateral marks that create unsafe debt."
      },
      {
        id: "what-is-recoverable-value",
        q: "What is recoverable value?",
        a: "Recoverable value is the discounted collateral value Avana uses for borrowing and liquidation decisions. It reflects what the LP position is worth after accounting for its unwind, sale, transfer, or settlement path under stress.\n\nRecoverable value is lower than theoretical LP value when the liquidation route includes slippage, price impact, range movement, volatility, fee handling, or limited route depth. Avana sizes borrowing against recoverable value because that is the value protecting the debt."
      },
      {
        id: "why-are-concentrated-liquidity-positions-treated-carefully",
        q: "Why are concentrated liquidity positions treated carefully?",
        a: "Concentrated liquidity positions change exposure quickly as price moves. A position can become mostly one asset, move out of range, stop earning fees, or require settlement at the full NFT-position level.\n\nAvana values concentrated positions by decomposing the actual position state instead of treating the NFT as a static token balance. The market then applies collateral factors, caps, and liquidation assumptions that match the position’s range, inventory, and unwind risk."
      },
      {
        id: "what-data-does-avana-monitor",
        q: "What market data does Avana monitor?",
        a: "Borrow Spokes monitor the signals that affect LP collateral quality: pool composition, token prices, asset correlation, volatility, liquidity depth, fee accrual, oracle reliability, and unwind cost.\n\nThose signals feed the market’s collateral treatment. Stable and highly correlated pools receive different limits than volatile or thinly traded pairs because their liquidation and slippage profiles are different."
      }
    ]
  },
  {
    ...
    questions: [
      {
        ...
        q: "How does borrowing work on Avana?",
        a: "Borrowing happens after the Borrow Spoke admits and values your LP collateral. When you request an asset, the spoke checks your remaining capacity, confirms the position stays healthy after the new debt, and draws liquidity from the Hub if the asset is available.\n\nThe borrowed asset is sent to the user, and the account records debt through the borrow-side accounting layer. Interest accrues over time, so health can change even if the collateral position itself is unchanged."
      },
      {
        id: "how-is-borrowing-capacity-calculated",
        q: "How is borrowing capacity calculated?",
        a: "Borrowing capacity comes from the risk-adjusted value of approved LP positions inside a Borrow Spoke. Avana reconstructs each position, prices the underlying exposure, applies pool-level risk treatment, and then applies the market’s collateral factor.\n\nThe Borrow Spoke reports that capacity to the Hub for enforcement. When a user has multiple approved positions in the same market, Avana aggregates their capacity while still valuing each position under its own pool, range, liquidity, and risk assumptions."
      },
      {
        id: "why-is-borrow-power-lower-than-lp-value",
        q: "Why is borrow power lower than my LP value?",
        a: "Borrow power is lower because Avana uses conservative collateral value, not the full LP mark. LP positions can lose value through price divergence, impermanent loss, range drift, liquidity stress, and liquidation slippage.\n\nThe difference between LP value and borrow power is the safety buffer. It protects the market by making sure debt is sized against recoverable collateral value rather than the best-case value shown during normal conditions."
      },
      {
        id: "what-assets-can-i-borrow",
        q: "What assets can I borrow?",
        a: "Borrowable assets are set per market. Avana borrow markets focus on liquid assets such as major stablecoins, GHO, ETH, BTC, and other assets that fit the market’s liquidity, cap, and risk configuration.\n\nThe interface shows the active borrow set for each market. A supported LP collateral type does not automatically unlock every borrow asset; each debt asset is part of the market configuration."
      },
      {
        id: "can-i-borrow-against-multiple-positions",
        q: "Can I borrow against multiple LP positions at once?",
        a: "Yes, when the positions are supported inside the relevant market. Avana can aggregate borrowing power across multiple approved LP positions in the same Borrow Spoke, giving the user one combined credit surface for that market.\n\nAggregation does not merge risk blindly. Each position still contributes capacity based on its own pool, range, liquidity, fees, and risk treatment, and the total account must remain above the required health threshold."
      }
    ]
  },
  {
    id: "supplying-earning",
    name: "Supplying & Earning",
    summary: "",
    questions: [
      {
        id: "how-do-lenders-use-avana",
        q: "How do lenders use Avana?",
        a: "Lenders supply assets such as ETH, BTC, GHO, USDC, USDT, or other supported assets into the lender-facing side of the protocol. That capital routes through the Hub to support borrowing across LP-collateral markets.\n\nLenders do not manage LP ranges, impermanent loss, or AMM-specific collateral operations. Borrow Spokes handle LP underwriting and liquidation logic, while lender capital powers the credit layer."
      },
      {
        id: "where-does-supplier-yield-come-from",
        q: "Where does supplier yield come from?",
        a: "Supplier yield comes from borrowers paying interest to access liquidity backed by LP collateral. Avana’s model combines the shared Hub base rate with spoke-level risk premiums tied to the LP markets being funded.\n\nThis means yield can reflect both normal utilization and the additional demand created by LP-backed borrowing. Rates still move with market conditions, available liquidity, utilization, and the risk profile of the underlying borrower markets."
      },
      {
        id: "do-lenders-manage-lp-collateral",
        q: "Do lenders manage LP collateral themselves?",
        a: "No. Lenders supply clean capital into supported lending markets. They are not choosing LP ranges, claiming LP fees, or deciding how a specific AMM position is unwound.\n\nThat separation is the point of the Lend Spoke. Lender capital supports specialized LP-backed borrower demand while Borrow Spokes keep collateral valuation, health checks, and liquidation handling isolated from the lender workflow."
      },
      {
        id: "what-protects-lender-capital",
        q: "What protects lender capital?",
        a: "Avana protects lender capital through overcollateralized borrowing, conservative LP valuation, dual-oracle checks, market caps, health-factor enforcement, and LP-specific liquidation paths. Each Borrow Spoke prices collateral according to its own pool risks before debt is drawn from Hub liquidity.\n\nThe protocol still carries DeFi risk: smart contract risk, oracle risk, liquidation execution risk, and market stress. Avana contains that risk by measuring LP collateral at the market level instead of treating every LP position like a simple token balance."
      }
    ]
  },
  {
    ...
    questions: [
      {
        ...
        a: "Health factor measures the relationship between risk-adjusted collateral value and outstanding debt inside a Borrow Spoke. The simplified model is adjusted collateral value divided by outstanding debt.\n\nAdjusted collateral value already includes Avana’s LP valuation, collateral factors, pool-level risk treatment, and recoverable-value assumptions. If health falls below the liquidation boundary, the position becomes eligible for liquidation."
      },
      {
        id: "what-affects-health-factor",
        q: "What affects my health factor?",
        a: "Health factor changes when collateral value changes, debt changes, fees are claimed, collateral is withdrawn, leverage is added, or market parameters update. LP positions can also change because token prices move, pool inventory shifts, fees accrue, or concentrated positions drift in and out of range.\n\nBorrowing more lowers health because debt rises. Repaying debt, adding approved collateral, or reducing exposure improves health because the account has more collateral support relative to debt."
      },
      {
        id: "how-does-liquidation-work",
        q: "How does liquidation work on Avana?",
        a: "Liquidation starts when an account’s health factor falls below the liquidation threshold. Aave handles debt accounting, health checks, and the liquidation entry point against the ERC-20 vault collateral.\n\nAvana handles the LP settlement behind that vault token. The Adapter burns the vault collateral, maps the liquidation back to the real LP position, and the settlement layer follows the market route: unwind, sell, auction, or transfer the backing position. Debt is covered first, then liquidator reward and settlement costs, with surplus handled by the market rule."
      },
      {
        id: "are-liquidations-permissionless",
        q: "Who can liquidate an unhealthy position?",
        a: "Liquidations are permissionless once a position crosses the liquidation threshold. Any eligible liquidator can repay the allowed debt amount and trigger the settlement path.\n\nLP collateral is harder to unwind than simple token collateral, so Avana also accounts for specialized liquidation coverage. Liquidators track the same risk state, vault-token mapping, route depth, and unwind assumptions used by the protocol."
      },
      {
        id: "what-happens-to-fees-during-liquidation",
        q: "What happens to LP fees during liquidation?",
        a: "Avana distinguishes accrued fees from principal liquidity. During liquidation, the settlement path applies accrued fees before principal liquidity when the market route supports that sequence.\n\nThis reduces how much of the core LP position has to be disturbed. If fee value does not cover enough debt, the settlement layer unwinds or transfers the required LP collateral according to that market’s liquidation path."
      }
    ]
  },
  {
    ...
    questions: [
      {
        id: "how-are-new-pools-added",
        q: "How are new LP pools added?",
        a: "New LP pools are added through market review and approval. A pool needs reliable underlying asset pricing, sufficient liquidity, clear unwind assumptions, and risk parameters that fit the collateral type.\n\nAvana starts with curated LP markets so valuation and liquidation stay clean. Broader pool coverage can expand over time as oracle support, liquidity depth, and risk data improve."
      },
      {
        id: "can-risk-parameters-change",
        q: "Can risk parameters change after launch?",
        a: "Yes. Risk parameters can change as market conditions change. A pool can become less liquid, more volatile, less correlated, or harder to unwind than it was when first listed.\n\nParameter updates can affect collateral factors, caps, supported pools, borrowable assets, pauses, liquidation thresholds, and liquidation bonuses. Users with open debt follow these updates because parameter changes affect borrow power and health factor."
      },
      {
        id: "where-can-i-follow-governance",
        q: "Where can I follow governance discussion?",
        a: "Most Avana governance updates and market discussion happen through Avana Labs on X at https://twitter.com/avana_labs. That channel covers new pools, market parameters, integrations, safety reviews, and product phases.\n\nLonger proposals, risk notes, and external governance threads are linked from Avana Labs when they are relevant. For large positions, users review the latest Avana Labs updates and the live market parameters before relying on a pool."
      },
      {
        id: "can-interface-fees-change",
        q: "Can interface fees change over time?",
        a: "Yes. Avana interface and service fees are operational settings for official frontends. They are separate from the core protocol mechanics that govern collateral, debt, oracle checks, and liquidation.\n\nWhen an interface fee applies, the transaction flow displays it before signature so users can distinguish frontend cost from gas, swap execution, protocol interest, and liquidation economics."
      }
    ]
  },
  {
    ...
    questions: [
      {
        id: "can-i-borrow-gho",
        q: "Can I borrow GHO through Avana?",
        a: "GHO is one of Avana’s supported borrow-side assets for markets that include GHO liquidity. When a market offers GHO, users can borrow it against approved LP collateral while the account remains above the required health threshold.\n\nBorrowing GHO works like any other debt asset in Avana: it increases outstanding debt, affects health factor, accrues interest, and is repaid in GHO plus any applicable costs."
      },
      {
        id: "can-gho-be-lp-collateral",
        q: "Can GHO be part of LP collateral?",
        a: "Yes. GHO can be part of LP collateral when the specific GHO pool is approved for an Avana market. The pool is evaluated like other LP markets: asset pricing, liquidity depth, correlation, token composition, caps, and liquidation route all matter.\n\nThis means GHO is not treated as a blanket approval for every pair. Avana supports the pool configuration, not just the asset symbol."
      },
      {
        id: "how-do-i-repay-gho",
        q: "How do I repay GHO debt?",
        a: "GHO debt is repaid in GHO plus accrued interest and any applicable costs. Once repayment confirms, Avana reduces the account’s outstanding debt and the health factor improves.\n\nAfter full repayment, collateral restrictions tied to that debt are released according to the market rules, allowing the LP position to be withdrawn or modified when no other debt remains."
      }
    ]
  },
  {
    id: "multiply-markets",
    name: "Multiply Markets",
    summary: "",
    questions: [
      {
        id: "what-is-avana-multiply",
        q: "What is Avana Multiply?",
        a: "Avana Multiply is the managed leverage workflow built on top of LP-backed credit. A user deposits supported LP collateral, Avana values it, and the interface can use that borrowing power to open approved loop or leverage exposure from one flow.\n\nInstead of manually borrowing, swapping, redepositing, and tracking debt across multiple tools, Multiply packages the position into one monitored workflow with collateral, debt, leverage level, health factor, and unwind controls shown together."
      },
      {
        id: "how-is-multiply-different-from-borrowing",
        q: "How is Multiply different from borrowing?",
        a: "Borrowing gives users assets they can use directly, as long as the account remains healthy. Multiply uses LP-backed borrowing power inside a managed strategy, where the borrowed capital is routed into approved exposure and tracked as one position.\n\nBecause Multiply adds leverage, it can increase capital efficiency and potential returns, but it also increases downside, liquidation risk, and sensitivity to price movement. The health view matters more, not less, when a position is looped."
      },
      {
        id: "do-i-keep-lp-while-using-multiply",
        q: "Do I keep my LP while using Multiply?",
        a: "Yes. The LP position remains the collateral base while the Multiply position is open. The user does not need to fully exit the AMM position just to create leveraged exposure.\n\nThe LP is still backing debt, so it remains subject to collateral rules and liquidation if the account becomes unhealthy. Users monitor collateral value, debt, leverage, and liquidation buffer throughout the life of the loop."
      },
      {
        id: "what-limits-maximum-leverage",
        q: "What limits maximum leverage?",
        a: "Maximum leverage depends on collateral quality, pool liquidity, volatility, oracle confidence, borrow asset, market caps, and liquidation assumptions. Stable or highly correlated pools can usually support different leverage limits than volatile or thin pools.\n\nAvana uses market-specific risk controls so leverage is constrained by the actual LP collateral behind the position rather than a flat multiplier applied to every pool."
      }
    ]
  },
  {
...
const schemaQuestionIds = faqCategories.flatMap((category) =>
  category.questions.slice(0, 3).map((question) => question.id),
)
