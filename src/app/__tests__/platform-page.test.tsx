...
describe("platform page", () => {
  ...
  it("restores the platform content while using the current app shell", () => {
    ...
    expect(screen.getByText("Why Avana")).toBeInTheDocument()
    expect(screen.getByText("Live position intelligence")).toBeInTheDocument()
    expect(screen.getByText("Automation with guardrails")).toBeInTheDocument()
    expect(screen.getByText("Fast, unified execution")).toBeInTheDocument()
    expect(screen.getByText("Built for active LPs")).toBeInTheDocument()
